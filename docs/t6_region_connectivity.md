...
## 7. Optional H&E overlay

If an image and a validated coordinate-to-image transform are available, pass
them explicitly:

```r
p_he_connectivity <- stg_plot_region_connectivity(
  connectivity,
  sample = "PC1_B1",
  background = he_raster,
  background_extent = c(xmin, xmax, ymin, ymax),
  x_transform = x_to_image,
  y_transform = y_to_image
)
```

The image transform must be established independently. An unverified overlay
can make correct geometry appear spatially misplaced.

## 8. Bridge QC and interpretation boundary

Radius selection must be declared before downstream expression testing.
A recommended workflow is:

1. predeclare a small set of physical radii;
2. nominate one primary scale;
3. display all scales;
4. inspect H&E for obvious cross-structure bridges;
5. save manual exclusions as a separate provenance table;
6. only then associate geometry with expression or phenotype.

stGrads deterministically constructs components and QC geometry. It does not
automatically claim that every component is one biological nest. In
particular, a large-radius component may bridge across stroma, lumina, or
separate epithelial structures through a chain of locally connected segments.

Recommended reporting language:

> We constructed expression-independent spatial components using a
> prespecified radius graph and quantified each segment's distance to the
> nearest buffer-union boundary as a relative rim-to-interior coordinate.
